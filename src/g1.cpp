...
typedef struct ASTAR_DATA
{
    ...
    ASTAR_DATA(unsigned node, float g, float h) : node(node), f(h + g), g(g)
    {
...
std::vector<unsigned> Graph::path(unsigned start, unsigned end)
{
    ...
    std::list<astar_t*> open;
    std::list<astar_t*> closed;

    
    aNodes.emplace_back(ASTAR_DATA(start));
    open.emplace_back(&aNodes.back());

    node_t& g = _adjList[end];

    while (open.size())
    {
        std::list<astar_t*>::iterator qa;
        ...
        for (unsigned successor = 0; successor < MAX_NODE_VERTICES; successor++)
        {
            ...
            if (!valid(sucN, true))
                continue;
            astar_t sucA = ASTAR_DATA(sucN, (*qa)->g + q.weights[successor], distance(suc, g));

            bool add = true;
            for (auto it = open.begin(); it != open.end(); it++)
            {
                if ((*it)->node == sucN && (*it)->f < sucA.f)
                {
                    add = false;
                    break;
                }   
